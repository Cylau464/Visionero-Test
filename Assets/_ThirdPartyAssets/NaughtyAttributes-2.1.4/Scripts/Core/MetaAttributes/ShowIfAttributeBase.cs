...
namespace NaughtyAttributes
{
    public class ShowIfAttributeBase : MetaAttribute
    {
        ...
        public Enum[] EnumConditions { get; private set; }
        ...
        public ShowIfAttributeBase(EConditionOperator conditionOperator, string enumName, params Enum[] enumConditions)
        {
            if(enumConditions == null || enumConditions.Length == 0)
                throw new ArgumentNullException(nameof(enumConditions), "This parameter must be an enum value.");

            ConditionOperator = conditionOperator;
            EnumConditions = enumConditions;
            Conditions = new string[1] { enumName };
        }
    }
}
