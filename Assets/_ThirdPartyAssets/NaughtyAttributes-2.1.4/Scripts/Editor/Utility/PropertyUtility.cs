...
namespace NaughtyAttributes.Editor
{
    public static class PropertyUtility
    {
        ...
        public static bool IsVisible(SerializedProperty property)
        {
            ...
            // deal with enum multiple conditions
            if (showIfAttribute.EnumConditions != null)
            {
                if (showIfAttribute.EnumConditions.Length > 0)
                {
                    Enum value = GetEnumValue(target, showIfAttribute.Conditions[0]);
                    List<bool> enumConditionValues = GetEnumConditionValues(value, showIfAttribute.EnumConditions);

                    if (value != null && enumConditionValues != null && enumConditionValues.Count > 0)
                    {
                        bool matched = GetConditionsFlag(enumConditionValues, showIfAttribute.ConditionOperator, showIfAttribute.Inverted);
        ...
        internal static List<bool> GetEnumConditionValues(Enum enumValue, Enum[] conditions)
        {
            List<bool> conditionValues = new List<bool>();

            foreach (Enum condition in conditions)
            {
                if (condition.GetType() != enumValue.GetType())
                {
                    string message = condition.GetType().Name + " condition type does not match enumValue type.";
                    Debug.LogWarning(message);

                    return null;
                }

                bool flag = enumValue.GetType().GetCustomAttribute<FlagsAttribute>() == null
                        ? condition.Equals(enumValue)
                        : enumValue.HasFlag(condition);
                conditionValues.Add(flag);
            }
